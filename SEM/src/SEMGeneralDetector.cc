      ...
      if (length>0) {
         ...
         for (int i=0;i<length;i++) {
            ...
            if ((!Ewindow) || ((tempel2.kinen >= Emin) && (tempel2.kinen <= Emax))) { // Note: boundaries of interval are INCLUSIVE
               OutputEID(output,tempel2,dataSEM);
               OutputTID(output,tempel2, dataSEM);
               OutputPID(output,tempel2, dataSEM);
               OutputEtot(output,tempel2, dataSEM,6);
               OutputEkin(output,tempel2, dataSEM,6);
               OutputEdeposit(output,tempel2, dataSEM,6);
               OutputEloss(output,tempel2, dataSEM,6);
               OutputTime(output,tempel2, dataSEM,3);
               OutputPosition(output,tempel2, dataSEM,6);
               OutputVertexPosition(output,tempel2, dataSEM,3);
               OutputDirection(output,tempel2, dataSEM,6);
               OutputDepth(output,tempel2, dataSEM,6);
               OutputMaxRadius(output,tempel2, dataSEM,6);
               OutputScatteringAngle(output,tempel2, dataSEM,6);
               OutputLogicalVolume(output,tempel2, dataSEM);
               OutputProcess(output,tempel2, dataSEM);
               OutputCategory(output,tempel2, dataSEM);
               output << G4endl;
            }
         }
      }
...
//	   G4String mkdir = (G4String) "mkdir ";
//	   G4int status = system((mkdir+dirname).c_str());
//         if (status) {
//            return 1;
//         }
//      }
